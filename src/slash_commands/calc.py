...
class SubmitCheckinView(discord.ui.View):
    expired_message = "This result is too old, please calculate your tier again to submit it."
    ...
    async def on_timeout(self):
        async with self.lock:
            # A submission already in progress owns the final status message.
            if self.attempted:
                return
            self.disable_all_items()
            if self.result_interaction is not None:
                try:
                    await self.result_interaction.edit_original_response(
                        content=self.expired_message, view=self
                    )
                except discord.HTTPException:
                    logging.exception("Could not update expired calculator result")
    ...
    async def submit(self, button, interaction):
        ...
        async with self.lock:
            ...
            if monotonic() >= self.expires_at or datetime.now(self.timezone).date() != self.created_date:
                ...
                await interaction.edit_original_response(content=self.expired_message, view=self)
                self.stop()
                return
            try:
